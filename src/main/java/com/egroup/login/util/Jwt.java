...
public class Jwt {
     ...
     * }
     */
    private final JWSHeader header=new JWSHeader(JWSAlgorithm.HS256, JOSEObjectType.JWT, null, null, null, null, null, null, null, null, null, null, null);
     ...
     */
	public Map<String, Object> validToken(String token,String SECRET) {
		...
		try {
			...
			if (jwsObject.verify(verifier)) {
				final JSONObject jsonOBj = payload.toJSONObject();
				// token校验成功（此时没有校验是否过期）
				resultMap.put("state", TokenState.VALID.toString());
				// 若payload包含ext字段，则校验是否过期
				if (jsonOBj.containsKey("ext")) {		
					final long extTime = Long.valueOf(jsonOBj.get("ext").toString());
